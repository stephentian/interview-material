...
"编译"与传统编译型语言的编译有些不同，是JavaScript引擎特有的"即时编译"（JIT）过程。

解析、编译、执行并非绝对割裂的线性阶段（实际存在流水线并行、懒解析、JIT 优化等），但 “解析→编译→执行” 是理解其核心逻辑的最佳框架。

#### 解析 parse

序列化(js => json) --> 抽象语法树AST --> 可执行代码

- 词法分析 `Lexical analyzer` (`Scanner`)
  - 扫描字符，将代码分解词法单元(tokens)，如关键字，操作符等
...
将 AST 抽象语法树转换为可执行的字节码或机器码。编译器（Compiler）负责执行编译过程。编译器遍历抽象语法树的节点，生成对应的字节码或机器码。

这个阶段更准确的名称是"预编译"（Precompilation），不是传统意义上的编译，而是JavaScript引擎内部的处理过程。

这个过程涉及技术：

- 即时编译 `JIT`: 发现某个函数或代码块被频繁执行，就会将其编译成高度优化的机器码。后续执行直接执行该段机器码。
- 隐藏类 `Hidden Class`：JS 对象属性可随时增删，V8 引擎会为对象创建隐藏类，记录其属性布局。这样访问属性时无需反复查找。
- 惰性解析 Lazy Parsing：JS 引擎不一开始就解析全部代码，而是执行到某段代码时才解析对应的 AST，减少初始加载的时间和内存开销，加快程序启动速度。

#### 执行 execute

js 引擎(Execution Engine) 负责执行过程。执行引擎会逐行执行字节码或机器码，并将结果输出到控制台或更新浏览器中的页面。

函数在这个阶段被调用，每次调用生成**执行上下文**。

- 垃圾回收：自动回收不再使用的内存。JavaScript 引擎（如 V8）主要采用“分代式垃圾回收”：将对象分为新生代和老生代，用不同策略高效清理无用对象，避免内存泄漏。

### 预编译

JS 引擎在执行代码前的准备工作（属于 “编译阶段” 的一部分），目的是为代码执行创建合适的环境。

- 词法分析（生成 Token）
- 语法分析（构建 AST）
- 创建执行上下文（提升，确定变量、函数、作用域等）

“预编译”不是 ECMAScript 标准术语，而是开发者用来描述“JS 在执行前先做一次快速编译/分析”的通俗说法。

### 提升 Hoist

提升在代码编译的语法分析触发，在创建执行上下文时将它们提前“注册”到词法环境（变量对象）中；有些文章称其为预编译；参考：[v8 parser.cc](https://github.com/v8/v8/blob/master/src/parsing/parser.cc)

注意：预编译是 “搭建执行环境” 的全过程，而提升只是这个过程中 “处理声明” 的一个具体动作。
...
JS 运行时全局作用域生成全局对象`GO`(`global object`)，函数会生成活动对象 `AO`(`Active Object`)。
...
- 词法环境（变量对象 `Variable Object`，VO）
- 作用域链(`Scope chain`)
- this

上下文环境结构两种：词法环境（`Lexical Environments`）和 变量环境（`Variable Environment`）
...
- catch 结构：一个 catch 结构块内也是自己一个词法环境
...
变量环境（VariableEnvironment） 是用来登记 `var function` 变量声明，词法环境（LexicalEnvironment）是用来登记 `let const class` 等变量声明。
...
也叫 静态作用域, 指作用域是在**函数声明**的位置确定，即代码写在哪里来决定的；
...
1. 避免在循环中创建闭包：在循环中创建的闭包很容易导致内存泄漏。

2. 及时释放闭包：在不再需要闭包时，可以将闭包设置为 null，从而释放其对外部变量和参数的引用。
...
非严格模式：JavaScript 诞生时的默认运行模式，为了兼容早期代码，存在大量 “不严谨、容错性高” 的语法和行为，容易产生隐性 bug（比如未声明变量就赋值不会报错）。

严格模式：`strict mode`

1. 全局严格：在脚本最顶部（注释后）添加 "use strict";（单 / 双引号均可）
2. 函数级严格：在函数内部最顶部添加 "use strict";
3. ES6 模块 / CommonJS 模块默认启用严格模式，无需手动声明

关键差异：

1. 变量声明
   - 非严格模式：变量声明未 var / let / const，会隐式挂载到 window 上为全局变量，不报错。
   - 严格模式：未声明变量赋值直接抛出 ReferenceError，提前暴露错误。
2. this 指向
   - 非严格模式：全局作用域 / 普通函数中，this 指向 window（浏览器环境）。
   - 严格模式：全局作用域 / 普通函数中，this 为 undefined（避免误操作全局对象）。
3. 函数参数禁止重复参数
   - 非严格模式：函数允许同名参数，后定义的覆盖前一个；
   - 严格模式：函数不允许同名参数，会抛出 SyntaxError 错误。
