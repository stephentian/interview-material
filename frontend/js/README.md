...
- 词法分析（生成 Token）
- 语法分析（构建 AST）
- 创建执行上下文（确定变量、函数、作用域等）

“预编译”不是 ECMAScript 标准术语，而是开发者用来描述“JS 在执行前先做一次快速编译/分析”的通俗说法。

### 提升 Hoist

提升在代码编译的语法分析触发，在创建执行上下文时将它们提前“注册”到变量对象中；有些文章称其为预编译；参考：[v8 parser.cc](https://github.com/v8/v8/blob/master/src/parsing/parser.cc)
