...
认为大多数对象的生命周期非常短暂，即从垃圾回收的视角来看，大多数对象在被分配后几乎立即变成不可访问状态。这一规律不仅适用于 V8 或 JavaScript，对大多数动态语言都成立。

将堆分为新生代和老生代，新生代用 Scavenge 算法快速回收短命对象，老生代用标记-整理处理长命对象。

JavaScript 垃圾回收算法分为两种：
