...
- 构造函数，即通过 js 的 [`apply`](../code/05.apply.js), [`call`](../code/06.call.js) 实现子类调用父类的属性、方法；
...
当访问当前对象的属性或者方法时，如果它本身找不到，就会去它`__proto__` 指向的对象的原型去寻找，直到到达原型链的终点 `null` 停止。这个链表结构称为原型链。
