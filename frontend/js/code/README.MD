...
- 用户多次点击按钮，时间间隔太短，只执行一次。
- 输入框实时搜索，如果输入间隔太短，只执行一次。
- 文本编辑同步，只同步最后一次。

节流 `throttle`: 单位时间内只运行一次。在这段时间重复触发，但是会按固定时间间隔执行。
...
function __const(data, value) {
  ...
  Object.defineProperty(window, data, {
      ...
      // value: value,  // value 和 get 互斥
      // writable: false, // writable 和 set 互斥. 防止常量值被更改
      get: function () {
        return value
      },
      set: function (newVal) {
...
js 不支持直接运算，只能使用 `+` 运算符触发 `toString`，以前调用方法会触发 `valueOf` 和 `toString`。

利用 闭包 和 `toString`/`valueOf` 实现，需要使用 `+` 触发 `toString`
