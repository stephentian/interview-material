...
### 模块化
    ...
    按**领域/职责**划分：
    - 通用基础模块：请求封装（axios封装）、状态管理（store）、工具函数库（utils）
    - 业务功能模块：用户模块、商品模块、订单模块、支付模块
    - 页面容器模块：首页、列表页、详情页、个人中心页
...
只有在第一次加载模块A时，模块A 的代码才会被执行，之后不管你再 `require('A')` 多少次都不会再执行模块A 的代码
...
javascript转译、代码压缩、css 预处理器、eslint、pretiier，等都建立在了AST这个巨人的肩膀上。
...
见 [Micro前端](../../architecture/MicroFrontend.md)
