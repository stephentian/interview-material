...
- **自主决策**：根据目标自主规划行动步骤
- **工具调用**：使用外部工具扩展能力边界
  > **💡 工具调用**（Tool Calling）：让AI能够调用外部函数或API来完成任务。
  > - **通俗解释**：给AI装上"手脚"，让它不只是说话，还能实际做事，比如查天气、订票、执行代码等。
  > - **例子**：用户问"北京明天天气怎么样"，AI不是凭记忆回答，而是调用天气API获取实时数据，再告诉用户结果。
- **记忆能力**：记住历史交互和状态
- **多步推理**：分解复杂任务为多个子任务
  > **💡 多步推理**：把复杂问题拆成多个小步骤，一步步解决。
  > - **通俗解释**：就像解数学题，先分析题目，列公式，计算，最后验证答案。AI也是这样一步步思考。
  > - **例子**：问"帮我订一张明天去上海的机票"，AI需要：1.理解日期"明天" 2.确定出发地 3.搜索航班 4.比较价格 5.完成预订，每一步都需要推理。
...
> **💡 Observation**（观察）：行动后获取的结果或反馈。
> - **通俗解释**：做完一件事后看到的结果，用来判断下一步该怎么做。
> - **例子**：AI调用天气API后，收到的"北京明天小雨，15-20℃"就是观察结果。
...
> **💡 短期记忆**：当前对话中的临时记忆，对话结束就消失。
> - **通俗解释**：就像打电话时你能记住对方刚才说的话，但挂了电话可能就忘了。
> - **例子**：你跟AI说"我叫张三"，AI在这次对话中会记住，但下次对话它就不知道你是谁了。

> **💡 长期记忆**：持久化存储的记忆，可以跨对话使用。
> - **通俗解释**：就像你写在笔记本上的内容，随时可以翻出来看。
> - **例子**：AI把重要的对话内容存到数据库，下次你问"上次我们聊了什么"，它能从长期记忆中找到。
