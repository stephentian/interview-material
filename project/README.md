...
使用 performance 监控性能，查看递归组件 script 脚本耗时，页面 render 渲染时间

递归：8.9s + 12s

优化：1.5s 以内

- 数据结构扁平化：把 tree 树转换为平铺数组，避免递归调用
- 虚拟列表，只渲染可见区域的节点。
  - 通过滚动条的位置来计算取哪些数据
  - clientHeight, scrollTop, clientWidth
...
TODO: 实现 微信小程序 vconsole network 监控功能
...
单纯用 setInterval 倒计时是不准时不可靠的。setInterval(fn, 1000) 并不保证每 1000ms 准时执行一次。
...
如果是秒杀场景，需要考虑前端倒计时必须和服务器时间一致，不能靠本地时间（因为本地时间可能会有时间差，用户可能修改。）
